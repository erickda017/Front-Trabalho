...
async function renderizarPaginaEmCanvas(
  ...
  try {
    ...
    // Libera a página individual se a API expor cleanup
    if (pagina && typeof pagina.cleanup === "function") {
      try { pagina.cleanup(); } catch (_) {}
    }
    // Libera os recursos do documento de forma compatível (evita estourar i.destroy is not a function)
    if (doc) {
...
// garantia nenhuma de que seja o Pix).
...
// bitmap e roda jsQR em cima. SÓ é chamado como fallback.
...
// Chama o Worker pra OCR (valor, vencimento, linha digitável).
...
// Processa uma página: chama o Worker primeiro e roda o scan de QR local caso necessário.
