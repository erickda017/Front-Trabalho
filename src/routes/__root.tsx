  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Algo falhou no carregamento. Tente novamente ou volte para o painel.
        </p>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    ...
    links: [
      ...
      {
        rel: "stylesheet",
        href: "https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,500..700&family=Inter+Tight:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500&display=swap",
      },
...
function RootShell({ children }: { children: ReactNode }) {
  return (
    <html lang="pt-BR">
      <head>
        <script dangerouslySetInnerHTML={{ __html: temaBootstrapScript }} />
        <HeadContent />
      </head>
      <body>
        {children}
...
function AuthGate() {
  ...
  if (session === undefined) {
    return (
      <div className="bg-background text-subtle grid min-h-screen place-items-center text-sm">
        Carregando painel...
      </div>
    );
  }
